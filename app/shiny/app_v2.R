    ...
    ,tabItems(
      ...
      tabItem(tabName = "add"
              ...
              ,box(
                ...
                ,textInput(inputId="add_id_txt" , label = "ID:" , placeholder= "ID of the observation to update")
...
server <- function(input, output) {
  ...
  observeEvent(input$add_button, {
    add_array <- c(
       as.numeric(input$add_id_txt)  #0
      ,as.numeric(input$add_preg_txt ) #1
      ,as.numeric(input$add_glucose_txt) #2
      ,as.numeric(input$add_blood_txt ) #3
      ,as.numeric(input$add_skin_txt) #4
      ,as.numeric(input$add_insulin_txt) #5
      ,as.numeric(input$add_bmi_txt) #6
      ,as.numeric(input$add_pedegree_txt) #7
      ,as.numeric(input$add_age_txt) #8
      ,as.numeric(input$add_cancer_txt) #9 
    )
    output$add_validation_message <-renderText(toString(add_array))
   ##POST('web:8080/users', body=toJSON(data.frame(
   ##  ID  = add_array[0]
  ...
  observeEvent(input$delete_button, {
    ...
    ##DELETE('web:8080/users', body=toJSON(data.frame(
    ##  ID  = delete_array[0])))
  })
  ...
  observeEvent(input$search_button, {
    ...
    ##GET('web:8080/users', body=toJSON(data.frame(
    ##  ID  = search_array[0])))
  })
  ...
  observeEvent(input$update_button, {
    ...
    ##PATCH('web:8080/users', body=toJSON(data.frame(
    ##  ID  = update_array[0]
    ##  ,Pregnancies = update_array[1]
    ##  ,Glucose = update_array[2]
    ##  ,BloodPressure = update_array[3]
    ##  ,SkinThickness = update_array[4]
    ##  ,Insulin = update_array[5]
    ##  ,BMI = update_array[6]
    ##  ,DiabetesPedigreeFunction = update_array[7]
    ##  ,Age = update_array[8]
    ##  ,Outcome = update_array[9])))
  }) 
  
  # ----------- PREDICT A VALUE  -----------
  observeEvent(input$predict_button, {
    predict_array <- c(
       as.numeric(input$predict_preg_txt ) #1
      ,as.numeric(input$predict_glucose_txt) #2
      ,as.numeric(input$predict_blood_txt ) #3
      ,as.numeric(input$predict_skin_txt) #4
      ,as.numeric(input$predict_insulin_txt) #5
      ,as.numeric(input$predict_bmi_txt) #6
      ,as.numeric(input$predict_pedegree_txt) #7
      ,as.numeric(input$predict_age_txt) #8
      ,as.numeric(input$predict_cancer_txt) #9 
    )
    output$predict_validation_message <-renderText(toString(predict_array))
    ##PREDICT('web:8080/users', body=toJSON(data.frame(
    ##  ID  = predict_array[0]
