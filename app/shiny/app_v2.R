...
server <- function(input, output) {
  ...
  observeEvent(input$radio, {
      ...
      # Get data 
      diabetes_original <-read.csv("diabetes.csv") # Fake data, to test with API  please comment this line
      
      # -- Using API method 
      ##   resp <- GET('web:8080/diabetes')
      ##   diabetes_original<- fromJSON(content(resp, as='text'))
  ...
  observeEvent(input$add_button, {
   ...
   ##  Pregnancies = add_array[0]
   ##  ,Glucose = add_array[1]
   ##  ,BloodPressure = add_array[2]
   ##  ,SkinThickness = add_array[3]
   ##  ,Insulin = add_array[4]
   ##  ,BMI = add_array[5]
   ##  ,DiabetesPedigreeFunction = add_array[6]
   ##  ,Age = add_array[7]
   ##  ,Outcome = add_array[8])))
  ...
  observeEvent(input$search_button, {
    ...
    # -- Using API method 
    ## resp_search <- GET('web:8080/users', body=toJSON(data.frame(
    ##  ID  = search_array[0])))
    ##  df_search<- fromJSON(content(resp_search, as='text'))
    
    
    
    ## Get tabla con el search 
    output$observation = renderDataTable({
      df <- read.csv("diabetes.csv") %>% head(5) # comment when trying the API 
      #df_search
  ...
  observeEvent(input$predict_button, {
    predict_array <- c(
      ...
      ,as.numeric(input$predict_cancer_txt) #8
    )
    output$predict_validation_message <-renderText(toString(predict_array))
    
    # -- Using API method 
    ## prediction_value <-PREDICT('web:8080/users', body=toJSON(data.frame(
    ##  Pregnancies = predict_array[0]
    ##  ,Glucose =predict_array[1]
    ##  ,BloodPressure = predict_array[2]
    ##  ,SkinThickness = predict_array[3]
    ##  ,Insulin = predict_array[4]
    ##  ,BMI = predict_array[5]
    ##  ,DiabetesPedigreeFunction = predict_array[6]
    ##  ,Age = predict_array[7]
    ##  ,Outcome = predict_array[8])))
    
    
    shinyalert(
      title = "The prediction of your observation is :  ",
      text = toString("Aqui hay que agregar la variable que traiga la predicción" ),
      size = "s", 
      closeOnEsc = TRUE,
      closeOnClickOutside = FALSE,
      html = FALSE,
